...
export class KeyboardTask {
  ...
  selectAll() {
    ...
    } else if (this.editor.selection?.anchor.path[0] === 0) {
      Transforms.select(this.editor, {
        anchor: Editor.start(this.editor, [0]),
        focus: Editor.end(this.editor, [0])
      })
    } else if (Editor.hasPath(this.editor, [1])) {
      Transforms.select(this.editor, {
        anchor: Editor.start(this.editor, [1]),
  ...
  clear() {
    if (this.editor.selection) EditorUtils.clearMarks(this.editor, !Range.isCollapsed(this.editor.selection))
  }
...
export const useSystemKeyboard = () => {
  ...
  const keydown = useCallback(action((e: KeyboardEvent) => {
    if (isHotkey('mod+,', e)) {
      e.preventDefault()
    ...
    if (isHotkey('arrowUp', e) || isHotkey('arrowDown', e)) {
      ...
      if (node?.[0].type === 'media') {
        ...
        if (isHotkey('arrowUp', e)) {
          ...
          Transforms.select(task.editor, Editor.start(task.editor, EditorUtils.findNext(task.editor, node[1])))
        }
        ReactEditor.focus(task.editor)
      }
    }
